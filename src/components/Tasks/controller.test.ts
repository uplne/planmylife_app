...
import { useTaskSchedulerStore, TaskSchedulerStoreDefault } from '../../store/TaskScheduler';
...
describe('Tasks controller', () => {
  ...
  describe('Fetching data', () => { 
    test('it should fetch data from firebase', async () => {
      ...
      expect(collection).toHaveBeenCalledWith('firestore', 'tasks/123-123/default');
      expect(where).toHaveBeenNthCalledWith(1, "assignedTimestamp", ">=", new Date(dayjs(selectedWeek).startOf('week').format()));
      expect(where).toHaveBeenNthCalledWith(2, "assignedTimestamp", "<", new Date(dayjs(selectedWeek).endOf('week').format()));

      expect(useTasksStore.getState().fillTasks).toHaveBeenCalledWith(TASKS_MOCK);
      ...
      expect(useTasksStore.getState().updateIsLoading).toHaveBeenNthCalledWith(1, DATA_FETCHING_STATUS.FETCHING);
      expect(useTasksStore.getState().updateIsLoading).toHaveBeenNthCalledWith(2, DATA_FETCHING_STATUS.LOADED);
    });
    ...
    test('it failed to fetch data', async () => {
      ...
      expect(useTasksStore.getState().updateIsLoading).toHaveBeenCalledTimes(2);
      expect(useTasksStore.getState().updateIsLoading).toHaveBeenNthCalledWith(1, DATA_FETCHING_STATUS.FETCHING);
      expect(useTasksStore.getState().updateIsLoading).toHaveBeenNthCalledWith(2, DATA_FETCHING_STATUS.ERROR);
    });
  });
